...
- **Draft**: Noni receives only the selected item, its exact Ryan brief, and referenced Nimi evidence. Dara returns a structured accept/revise review; deterministic code permits at most one issue-bound revision, persists the full trace, and derives effect proposals only from the exact accepted text.
