...
def reset_nimi_skill_trace(callback_context: Any) -> None:
...
def nimi_analysis_skill_context() -> str:
    ...
    resources = {Path(path).name: (NIMI_SKILL_ROOT / path).read_text() for path in NIMI_SKILL_REFERENCES}
    sections = ["Nimi preloaded evidence analyst runtime skill:", (NIMI_SKILL_ROOT / "SKILL.md").read_text()]
...
def bootstrap_nimi_skill_context(callback_context: Any) -> None:
...
def guard_nimi_skill_tool(tool: Any, args: dict[str, Any]) -> None:
...
def record_nimi_skill_tool(tool: Any, args: dict[str, Any], tool_context: Any) -> None:
