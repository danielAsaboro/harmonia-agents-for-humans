...
The Harmonia Console combines operator conversation with a living editorial canvas. Bedrock may select and arrange a fixed vocabulary of trusted A2UI components; it cannot generate JavaScript, HTML, CSS, arbitrary React components, or executable callbacks.
...
`POST /api/chat/stream` creates a tenant-scoped DynamoDB run. Each schema-validated NDJSON record receives a monotonic sequence number and is persisted before delivery. A disconnected client resumes with:

```text
GET /api/chat/runs/{runId}/events?after={sequence}
```

This is durable transport streaming. It does not claim direct provider-token streaming when the bounded provider invocation returns a complete result.
