...
| Noni | platform-native writing and revision | one deterministically selected Temi item, its exact Ryan brief, referenced Nimi evidence, and the local `noni-writing-skills` guidance | research, search trends, access Memory Bank, inspect the rest of the plan, create facts, approve, publish |
...
drafting. Its toolset exposes only `load_skill` and `load_skill_resource`; it has no research, trend,
Memory Bank, workspace-data, approval, or effect tool. The ten references cover thought leadership,
hooks and introductions, MECE structure, case studies, storytelling, bad-content diagnosis,
persuasion, outlining, titles and headlines, and convincing content. Skill guidance shapes writing
but never counts as evidence for a factual claim.
