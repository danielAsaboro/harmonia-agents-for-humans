...
Sign in to Harmonia and choose **Architecture** in the dashboard rail, or open `/dashboard/architecture`. The initial System overview intentionally keeps most groups collapsed. Search for an agent, model, route, tool, skill, store, service, or stage; use presets for a guided view; select any node for its source, authority, state, and evidence details.

Zoom, pan, fit, reset, expand, and collapse controls are available on desktop. The URL stores the preset, selected node, expansion, filters, and search query so the current explanation can be shared. Pan and zoom stay local to avoid noisy links.
...
The palette follows the blueprint: cyan infrastructure, violet agents, amber policy and human approval, green executed or independently verified effects, red uncertainty or blocked authority, and gray external systems.
