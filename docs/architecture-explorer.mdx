...
Edge semantics never rely on color alone. Labels, marker words, and line patterns distinguish durable transitions, delegation, human decisions, effects, independent verification read-back, read-only retrieval, AgentCore Memory retrieval, telemetry, and blocked or uncertain paths. Open the persistent Legend for the current key.

## Agent hierarchy and authority

Harmonia delegates bounded evidence analysis to Nimi, strategy to Ryan, and editorial operationalization to Temi. Nimi loads one allow-listed filesystem analysis skill; when an exact research request exists, public Google Search or private Amazon Bedrock Search runs in an isolated child agent and retains native grounding metadata. Deterministic code validates the approved output plan, then runs separate typed Noni production and Dara review batches with at most one issue-bound revision before sealing immutable artifacts. Maya provides bounded AI SDK presentation and Nova provides skill-first read-only answers. The coordinator delegates; it cannot approve or publish. No cognitive role can approve, publish, mutate credentials or external calendars, perform destructive work, or execute a paid effect.

See [Strands agent platform](./agent-platform) and [tool contracts](./tool-contracts).

## Workflow and state ownership

The workflow is collect_sources → extract_sources → understand with Nimi → strategize with Ryan → await strategy approval → plan with Temi → Noni/Dara typed artifact production → await effect approval → publish/export/render → verify → learn. DynamoDB is the durable source of truth and SQS owns asynchronous delivery and retry. AgentCore Runtime sessions are ephemeral cognition; AgentCore Memory stores only exact-scope eligible facts and never authority.
...
Filesystem skills contain reusable project-owned methods. They are loaded through Strands
`SkillToolset` loaders, have explicit resource allow-lists, and never count as factual evidence.

## Tool system

Runtime tools retrieve or ground data. Google Search and Amazon Bedrock Search are tools inside isolated
...
signal tools are distinguished from tenant-scoped workspace reads. Managed Strands
...
The explorer records the checked-in allocation: Claude Haiku 4.5 for intent, presentation and liaison, Claude Sonnet 4.6 for strategy, planning, writing and review, and Nova 2 Lite for visual analysis. Noni and Dara link to their bounded filesystem method skills. Status badges distinguish offline contract verification from pending authenticated live evidence.

See [models, cost, and evaluation](./models-cost-evaluation).

## Route families

Routes are grouped by responsibility: auth and tenancy; jobs and decisions; chat and AI SDK; content and monitoring; Calendar and OAuth; and the service-authenticated internal worker API boundary. The detail drawer lists representative checked-in endpoints rather than pretending the group is exhaustive.

## Observability and cost controls

W3C trace context crosses SQS and internal calls. Spans contain identifiers, stages, role/model labels, counts, and outcomes only—not prompts, responses, transcripts, drafts, media, or hidden reasoning. Deterministic reservations and immutable usage records support job and workspace budget guards.
...
Validation rejects duplicate IDs, dangling endpoints, parent cycles, private paths, prohibited agent authority, durable AgentCore Runtime ownership, effects without approval, and effects without verification. Keep source links under `src/`, `agent/`, or `docs/`.
