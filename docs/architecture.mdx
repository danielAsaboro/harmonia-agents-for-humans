...
flowchart TB
    ...
    subgraph AgentPlatform["Vertex AI Agent Engine"]
        ...
        SPECIALISTS[Single-turn specialists<br/>Nimi analysis · Ryan strategy]
        DRAFTFLOW[Flo content engine<br/>Temi plan → Noni ↔ Dara loop]
...
Harmonia, the ADK root coordinator, delegates evidence analysis to Nimi (`nimi_analyst`) and strategy/content planning to Ryan (`ryan_strategist`). Production uses Flo (`flo_content_engine`), an `AgentTool` around a deterministic `SequentialAgent`: Temi creates a typed editorial plan, then a bounded `LoopAgent` runs Noni writing and Dara review for at most two passes. Deterministic stage code derives effect proposals from exact reviewed text; Firestore remains durable truth between Pub/Sub stages.
...
| Ryan strategist | Gemini 3.5 Flash | 0.4 / 2,048 | goals, pillars, campaign/content-plan direction | Nimi evidence + brand goals | write final copy, approve, publish |
| Temi editorial planner | Gemini 3.5 Flash-Lite | 0.1 / 1,024 | typed objectives, source refs, format, priority | Ryan/Nimi grounded state | write copy, approve, schedule externally, publish |
| Noni copywriter | Gemma 3 12B IT Vertex endpoint | 0.8 / 2,048 | references and X length | Temi plan + session state | create facts, approve, publish |
...
No agent has publishing, approval, credential, or destructive tools. Temi creates editorial plans; deterministic stage code derives action proposals only from Dara-reviewed text and adds packs and media actions before the existing human gate.
