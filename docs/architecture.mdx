...
Cloud Scheduler invokes the private worker's `/durable/tick` endpoint once per minute with an OIDC
service account that has only Cloud Run invocation authority. A Firestore transaction grants one
55-second workspace lease per minute. The tick runs scheduled-effect dispatch, proactive checks, and
...
remain development-only and disabled in deployed revisions.
