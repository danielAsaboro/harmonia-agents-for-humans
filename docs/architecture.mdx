...
flowchart TB
    ...
    subgraph Worker["ADK worker (FastAPI / Cloud Run)"]
        ...
        DRAFTFLOW[Flo workflow<br/>Nimi: Gemma 3 endpoint<br/>Dara: Flash → Temi: Flash-Lite]
...
<Warning>
  Distinct role routing, the Gemma adapter, direct video attachment, budget enforcement, usage normalization, and metadata-only trace behavior are verified offline. Authenticated Gemini/Gemma execution and exported Cloud Trace correlation remain unverified until separate live evidence is captured.
</Warning>
