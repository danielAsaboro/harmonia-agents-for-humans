...
Harmonia, the ADK root coordinator, delegates evidence analysis to Nimi (`nimi_analyst`) and agentic strategy/content planning to Ryan (`ryan_strategist`). Firestore persists Nimi evidence and Ryan's digest-bound proposal across deterministic stages. After independent human strategy approval, Flo (`flo_content_engine`) runs Temi planning followed by the bounded Noni/Dara production loop. Deterministic stage code derives effect proposals from exact reviewed text; Firestore remains durable truth between Pub/Sub stages.
