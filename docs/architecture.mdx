...
flowchart TB
    ...
    subgraph AgentPlatform["Vertex AI Agent Engine"]
        ...
        DRAFTFLOW[Flo production loop<br/>selected item: Noni ↔ Dara]
...
Harmonia, the ADK root coordinator, delegates evidence analysis to Nimi (`nimi_analyst`), strategy/content briefs to Ryan (`ryan_strategist`), and editorial operationalization to Temi (`temi_editorial_planner`). After independent human strategy approval, Temi proposes the complete four-week plan. Deterministic code validates it, computes its digest and lineage, persists it in Firestore, and selects exactly one eligible item. Flo (`flo_content_engine`) then runs only the bounded Noni/Dara production loop for that item. Deterministic stage code derives effect proposals from exact reviewed text; Firestore remains durable truth between Pub/Sub stages.
...
| Dara editor | Gemini 3.5 Flash | 0.2 / 2,048 | preserve IDs/references | Noni drafts + session state | create authority, approve, publish |
...
No agent has publishing, approval, credential, external-calendar, or destructive tools. Temi proposes editorial plans; deterministic code owns validation, digest, persistence, item selection and lifecycle, and every external scheduling/effect mutation. It derives action proposals only from Dara-reviewed text and adds packs and media actions before the existing human gate.
