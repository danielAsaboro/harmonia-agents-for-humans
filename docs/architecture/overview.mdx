...
flowchart TB
    ...
    subgraph AgentPlatform["Vertex AI Agent Engine"]
        ...
        DRAFTFLOW[Deterministic artifact production/review<br/>approved output plan: Noni ↔ Dara]
...
Harmonia, the ADK root coordinator, delegates evidence analysis to Nimi (`nimi_analyst`), strategy/content briefs to Ryan (`ryan_strategist`), and editorial operationalization to Temi (`temi_editorial_planner`). After independent human strategy approval, Temi proposes the output plan. Deterministic code validates its digest and lineage and persists it in Firestore. The worker then invokes Noni (`noni_content_producer`) and Dara (`dara_artifact_reviewer`) as separate typed ADK specialists in a deterministic batch loop: one complete artifact batch, one complete review, and at most one issue-bound revision plus final review. Deterministic code seals accepted artifacts with exact digests and derives effects from those immutable identities; Firestore remains durable truth between Pub/Sub stages.
...
| Noni content producer | Gemini 3.5 Flash | 0.8 / 8,192 | exact output-plan coverage, provenance, grounding, and format schemas | approved output plan + exact Nimi evidence + brand constraints | redefine strategy, access Memory Bank, create facts, approve, persist, publish |
| Dara artifact reviewer | Gemini 3.5 Flash | 0.2 / 8,192 | complete grounding, brief, brand, format, CTA, safety, and clarity checks for every artifact | exact Noni batch + immutable production input | rewrite content, create facts or authority, approve, persist, publish |
...
For each job, Harmonia loads the exact sealed manifest and its normalized artifacts. Authorized video and audio are supplied to Gemini for timed extraction; documents, web pages, and pasted text use typed page, paragraph, section, or character-range locators. Nimi receives only those identified segments and may propose clip moments only from real time ranges. It loads its analysis skill and relevant method references before returning one strict `SourceAnalysis`. Optional current public or private context is fetched only by an isolated request-bound search agent. Runtime code validates the real trace and native grounding metadata, then rejects mismatched digests, invented references, non-verbatim quotes, invalid locator bounds, unsupported visual claims, and authority language before storing the full analysis, research lineage, metadata, and canonical digest. Media bytes and source URIs are excluded from trace content.

No agent has publishing, approval, credential, external-calendar, or destructive tools. Temi proposes output plans; Noni proposes typed artifacts; Dara proposes structured review decisions. Deterministic code owns validation, canonical digests, persistence, the one-revision limit, and every external scheduling/effect mutation. It derives actions only from sealed accepted artifacts. X post/thread and LinkedIn publication plus paid-media actions wait for human approval; immutable artifact export, images, and ffmpeg renders remain internal and separate from publication. A content-pack artifact is a manifest over constituent artifact IDs and digests and cannot export until every constituent's JSON and Markdown bytes independently verify.
