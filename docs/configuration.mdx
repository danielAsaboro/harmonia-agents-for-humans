...
| `MALWARE_SCANNER_URL` | production uploads | deployed automatically | IAM-private Harmonia ClamAV endpoint; upload completion stays fail-closed when absent |
| `MALWARE_SCANNER_TOKEN` | production setup override | generated automatically | Secondary scanner credential mounted from Secret Manager; never exposed to clients |
...
| `HARMONIA_TIME_ORIGIN` | no | host time | Optional UTC origin for replayable or accelerated workflows; does not change the operating-system clock |
| `HARMONIA_TIME_SPEED` | no | `1` | Server-time multiplier applied relative to `HARMONIA_TIME_ORIGIN`; must be positive |
...
| `STRATEGIST_TIMEOUT_SECONDS` | no | `300` | Bounded Ryan strategy invocation timeout |
...
Every role model variable also accepts a matching `*_TIMEOUT_SECONDS` override, for example `ANALYST_TIMEOUT_SECONDS` and `PLANNER_TIMEOUT_SECONDS`. The default is 120 seconds unless a role declares a larger bounded default. Timeouts limit one specialist invocation; they do not authorize an additional effect attempt.
