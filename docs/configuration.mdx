...
```bash
cd agent
HARMONIA_REAL_EVAL=1 ./.venv/bin/python -m harmonia_agent.evaluation_runner \
  --evalset ../../resources/private-evals/harmonia.evalset.json \
  --output ../../submission/evidence/harmonia-eval-results.json
```

Convert recorded pass rate, p95 latency, and immutable cost records into a selection report:

```bash
./.venv/bin/python -m harmonia_agent.evaluation_report \
  --input ../../submission/evidence/role-evaluation-records.json \
  --minimum-pass-rate 0.95 --format markdown
```

A cheaper model is ineligible when it misses the quality floor. A model with unknown pricing is
also ineligible. Change a role's catalog default only after a recorded comparison uses the same
policy and pricing versions. Mock or scripted runs never count as model-promotion evidence.
