...
| `MALWARE_SCANNER_URL` | production uploads | — | Private malware-scanner endpoint; deployment refuses to proceed when absent |
...
| `HARMONIA_ENABLE_RESIDENT_LOOPS=1` | Development-only opt-in for the legacy scheduler, proactive scan, and Telegram long-poll threads. Disabled by default; these threads are not a durable production scheduler. |
