...
`infra/setup.sh` provisions a generated `malware-scanner-token` and a dedicated scanner service
account. `infra/deploy.sh` builds and deploys the repository's ClamAV scanner as an IAM-private Cloud
Run service, grants invocation only to the web runtime, and mounts the shared token as a second
application-level authentication boundary. Upload completion remains fail-closed for unavailable,
timed-out, malformed, truncated, or incomplete scan results. Capture the ClamAV engine and definition
version from a real clean scan before presenting uploaded-media operation as production evidence.
...
`REGION` controls regional infrastructure placement, not end-to-end data residency. Setup verifies the Firestore database and Agent Engine staging bucket location and applies a Pub/Sub message-storage policy with in-transit enforcement. Agent Engine, Memory Bank, and Veo resources must match that region. Gemini 3.7 Flash cognition and transcription explicitly use Vertex `global`; they do not fall back to the Developer API. Transcription uses inline bytes for small inputs and private temporary GCS objects for larger inputs (`MEDIA_OUTPUT_BUCKET` or `GCS_BUCKET`). Lyria uses a global endpoint and remains disabled unless `ALLOW_GLOBAL_LYRIA=true`. These controls are not a regulatory certification or a guarantee that Gemini inference stays in one region.
