...
The deployed worker does not depend on process-resident scheduler or proactive supervisors. Cloud Scheduler drives the durable tick, and Firestore workspace leases plus stage/effect claims make concurrent delivery safe. Keep `HARMONIA_ENABLE_RESIDENT_LOOPS` disabled outside development.
