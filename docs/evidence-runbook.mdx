...
The complete capture must correlate the approval action, idempotency key, effect receipt, verification readback or artifact digest, cognition usage, cost records, and trace lineage. Cognition must share a trace with a lifecycle event; approval, effect, and verification must share their action trace; replay must be a distinct traced attempt. A zero trace ID, verification timestamp before effect execution, receipt mismatch, unknown observed cost, mock provenance, or raw private field fails closed.
