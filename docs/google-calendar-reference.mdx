...
| Token storage | Workspace-scoped DynamoDB connection document; server-side only |
...
The first sync provisions a secondary calendar named `Harmonia Content Calendar`. DynamoDB records a provisioning claim before calling Google.
