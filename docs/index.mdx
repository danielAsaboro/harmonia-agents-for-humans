...
Harmonia documents implemented contracts separately from authenticated deployment evidence. Repository tests and local emulator runs prove local behavior only; provider and cloud claims require captured authenticated evidence. The currently reachable ECS Fargate revision is a **web-only preview** and does not prove a deployed worker, Scheduler, AgentCore Runtime invocation, AgentCore Memory retrieval, or live publish. Use the [evidence runbook](/evidence-runbook) before presenting deployment, model, publishing, or verification claims.
