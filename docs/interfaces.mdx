...
The same intents over the official Bot API, scoped to an allow-listed chat id (`TELEGRAM_ALLOWED_CHAT_ID`) using a bot token (`TELEGRAM_BOT_TOKEN`). Approvals require an explicit inline-button callback — a plain "approve" message only lists pending actions. The bot never echoes its own credentials into chats.
