...
The internal writer authenticates before parsing and rejects records whose workspace or brand differs from the active tenant. Browser queries derive tenancy from the authenticated session. Records receive a 30-day deletion timestamp; production must configure a DynamoDB TTL policy on `retentionDeleteAfter` for the `agent_activity` collection group.
