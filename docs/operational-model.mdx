...
The dashboard's operational projection is also a control surface, not workflow truth. It compiles Firestore jobs, approvals, connection health, resident attention, and unknown effects into a reconnectable needs-you inbox and compact job shells. Versioned pause, resume, and cancellation commands update durable desired state; workers enforce that state when admitting new stage or effect claims.
