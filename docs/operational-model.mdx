...
The autonomous core is the persisted job/effect-command pipeline driven by Pub/Sub. The legacy in-process scheduler, proactive scan, and Telegram long-poll threads are disabled unless `HARMONIA_ENABLE_RESIDENT_LOOPS=1` is explicitly set for development. A production scheduled-job tick source is not configured by this repository and must not be inferred from those development loops.
