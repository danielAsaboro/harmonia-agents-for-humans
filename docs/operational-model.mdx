...
| extract_sources | agentic cognition inside deterministic workflow | Transcribe speech and Nova visual observations | normalized artifact schema, exact source digest, typed locators, budget, timeout, and visible failure contract |
...
The verifier computes consistency from redacted records; authenticated ECS Fargate, AgentCore Runtime, DynamoDB, SQS, model, effect, and verification exports establish provenance. Until that run exists, numeric KPI values remain explicitly pending rather than estimated or fabricated.
