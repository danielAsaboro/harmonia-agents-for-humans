...
For video jobs, Harmonia delegates the formatted transcript to Nimi (`nimi_analyst`) for clip-worthy moments (timestamps + hook + exact quote), trend angles, and meme angles. Ryan (`ryan_strategist`) turns grounded evidence or an operator brief into strategy and content-plan direction; brief-only concepts use zero timestamps because there is no media timeline.
