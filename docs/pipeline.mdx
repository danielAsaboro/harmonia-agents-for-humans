...
Hard gate for X and LinkedIn publishing and paid Veo/Lyria generation. Approvals arrive through the dashboard queue, chat ("approve job \<id\>"), or a Telegram inline-button callback — all through the same decision engine. Verified content-artifact export, Gemini image generation, and local ffmpeg rendering are internal actions with `approvalState: not_required`.
