...
- Ryan receives one filesystem `SkillToolset` restricted to approved method references. Native Google Search is isolated inside a separate request-bound ADK search agent so it never shares a model request with Ryan's loaders.
- Nova receives a read-only ADK `SkillToolset`; Noni receives bounded writing-skill and grounded-research tools; Dara receives only its filesystem editing-skill loaders. Other core specialists are tool-free.
- Callbacks attach authorized media evidence and reset, record, and validate real Ryan, Noni, Dara, and Nova tool traces.
