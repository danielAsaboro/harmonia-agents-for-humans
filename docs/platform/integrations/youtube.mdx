...
The source registry validates the URL and records source rights. During `extract_sources`, the worker resolves public metadata through YouTube/oEmbed, obtains authorized media, and uses Gemini to produce digest-bound timed segments. Those segments and authorized video evidence join the same normalized source bundle used for documents, web pages, pasted text, uploads, and pinned brand-library snapshots.
