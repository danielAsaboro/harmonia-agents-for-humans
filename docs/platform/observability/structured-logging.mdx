...
Python modules use named `logging` loggers for service startup, stage outcomes, retry decisions, provider failures, scheduling, proactive scans, and notification errors. ECS Fargate captures stdout and stderr. Correlation identifiers in surrounding telemetry and persisted records connect logs to a job or operation.
...
Redaction helpers, typed failures, and safe telemetry attributes have automated coverage. Production logging claims require redacted ECS Fargate log entries correlated with persisted workflow records.
