...
| `publish_pulse` | hourly | Fresh metrics for posts <48h old; likes spikes fire momentum proposals immediately |
...
- **Trend, calendar-gap, and recycle ideation**: Harmonia delegates these judgment tasks to Ryan (`ryan_strategist`). Calendar gaps are grounded in *your* operator goals and best-performing posts. Under `HARMONIA_MOCK_AI=1` all specialist outputs come from deterministic fixtures with explicit delegation traces.
...
The Telegram briefing push requires `TELEGRAM_BOT_TOKEN` + `TELEGRAM_ALLOWED_CHAT_ID`; without them it is skipped and logged, never an error.
