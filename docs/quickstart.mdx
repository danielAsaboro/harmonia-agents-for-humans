...
| Authenticated cognitive job | Invoke configured Google identity, Agent Engine, Gemini/Gemma, and managed memory boundaries | Only the providers captured in that specific run |
...
Watch the job progress through ingest → transcribe → understand → draft and stop at the approval boundary. Review the persisted action and use the trusted approval control. In chat, an approval phrase opens that same deterministic confirmation; model-parsed text cannot authorize the effect by itself.
