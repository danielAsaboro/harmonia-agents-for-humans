...
| `PUBSUB_PRODUCTION_TOPIC` | `harmonia-production` | Web and agent runtime | Mandate-authorized paid production-operation wake transport. |
...
| `STRATEGIST_TIMEOUT_SECONDS` | `300` | ADK worker | Ryan's bounded strategy invocation timeout. Other roles accept the corresponding `<ROLE>_TIMEOUT_SECONDS` variable and default to 120 seconds. |

Production infrastructure must provision all three before enabling their corresponding dispatch paths. `infra/setup.sh` creates the topics with the selected-region message storage policy.
