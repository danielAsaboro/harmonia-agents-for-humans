...
Run the TypeScript and Python suites, lint, type checking, and production build. Inspect `/dashboard/autonomy` locally. Then deploy deliberately, confirm IAM/OIDC and tenant isolation, and capture one authenticated cycle. Until those checks exist, keep the status **not deployed** and **not evidenced**.
