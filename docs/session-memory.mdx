...
description: "How Harmonia separates invocation state, bounded conversation continuity, DynamoDB truth, and cross-session AgentCore Memory facts."
---
...
<CardGroup cols={2}>
  ...
  <Card title="DynamoDB" icon="database">Authoritative durable workflow, approval, effect, cost, and failure state.</Card>
  <Card title="AgentCore Memory" icon="brain">Evidence-linked decisions and verified outcomes that may inform later jobs.</Card>
</CardGroup>
...
|---|---|---|---|
| Strands invocation state | one managed call | discarded after validation | no |
| AgentCore Runtime session | deterministic hash of `{workspace}:{user}:{job}:{operation}:{specialist}` | retained by AgentCore Runtime for restart recovery under provider/deployment lifecycle policy | no |
...
| DynamoDB job and audit records | workspace + brand + entity | deployment policy | yes, only through typed deterministic contracts |
| AgentCore Memory | exact workspace + brand | provider/deployment policy | no |
...
AgentCore Memory writes accept only typed candidates linked to durable DynamoDB evidence.
...
<Info>DynamoDB remains the recovery source after a crash or SQS redelivery. AgentCore Memory improves later judgment; it cannot reconstruct or overwrite workflow truth.</Info>

<Warning>The scope, eligibility, and failure contracts below are verified offline. Do not claim live
cross-job memory until a real AgentCore Runtime write, a later same-scope retrieval, and a different-scope
negative retrieval have been captured from the authenticated deployment.</Warning>

Each cognitive operation derives a stable, non-content-bearing AgentCore Runtime session ID. A restarted
...
allowing session state to authorize, replay, or advance the DynamoDB workflow. Durable stage leases
