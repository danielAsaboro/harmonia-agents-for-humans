...
- Temi returns an agentic proposal only. Deterministic code validates it against the approved Ryan strategy and supplied capabilities, computes its canonical digest, persists the complete plan, and selects exactly one eligible item. Noni receives only that selected item, its exact Ryan brief, and referenced Nimi evidence. Firestore persists Noni's original/revision drafts, Dara's structured reviews, the exact accepted draft, and a canonical trace digest before any approval-bound effect can advance.
