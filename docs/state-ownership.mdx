...
|---|---|---|---|---|---|---|
| Invocation state | one managed cognition call | until the call ends | web/worker seeds validated JSON; Strands `output_key` and event `state_delta` add handoffs | coordinator and selected specialists | discarded after validation | no; typed transport only |
| Managed Strands invocation | hashed workspace/user/job/operation/specialist | restart-resumable cognitive operation | AgentCore Runtime event lifecycle only | managed runner for that operation | provider/deployment session lifecycle; job erasure remains DynamoDB/storage scoped | no; never workflow authority |
| DynamoDB job | workspace + brand + job | workflow plus explicit retention policy | authenticated web internal routes and deterministic state machine | dashboard, worker, chat, evidence collector | terminal jobs receive a 90-day deletion deadline; holds block erasure | **yes** for stage, status, Temi's immutable planning snapshot and digest, the complete Temi editorial plan and digest/lineage, selected item lifecycle, artifacts, budget, and failure |
...
- Retrieved session objects are read-only. Harmonia copies serializable input into a new seed object only on deterministic session creation, consumes only event `state_delta`, and validates required output keys. DynamoDB—not the managed session—governs recovery and authority.
- DynamoDB mutations go through authenticated typed routes and deterministic transitions. Models cannot write approval, receipt, verification, credentials, or budget authority.
- Nimi returns an agentic analysis proposal only. Deterministic code validates it against the exact typed source package, computes its canonical digest, and persists the complete `SourceAnalysis` and provenance in DynamoDB before Ryan can run. AgentCore Memory facts remain advisory inputs and never become authority.
- Deterministic code first persists a canonical planning snapshot from tenant-scoped DynamoDB truth. Temi can read only that exact snapshot and returns an agentic proposal bound to its digest. Deterministic code validates the proposal against the snapshot and approved Ryan strategy, computes the plan digest, persists the complete plan, and selects exactly one eligible item. Noni receives only that selected item, its exact Ryan brief, and referenced Nimi evidence. DynamoDB persists Noni's original/revision drafts, Dara's structured reviews, the exact accepted draft, and a canonical trace digest before any approval-bound effect can advance.
...
- AgentCore Memory context is advisory and never authorizes a plan item, selected item, schedule, approval, policy exception, or effect.
...
the job document with every DynamoDB subcollection. The final tombstone contains IDs, actor, reason,
