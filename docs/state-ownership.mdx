...
- Temi returns an agentic proposal only. Deterministic code validates it against the approved Ryan strategy and supplied capabilities, computes its canonical digest, persists the complete plan, and selects exactly one eligible item. Noni receives only that selected item, its exact Ryan brief, and referenced Nimi evidence.
