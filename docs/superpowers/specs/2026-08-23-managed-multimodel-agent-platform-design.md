...
Harmonia currently has a typed Strands Agents SDK hierarchy, but every role resolves to one configured Gemini model. The team shares invocation-local state, while DynamoDB carries durable workflow state. Audio is sent to Gemini for transcription and images can be generated, but the analyst reasons over transcript JSON rather than the original video, audio, and visual frames. The application records business events and receipts, but it has no OpenTelemetry traces, managed AgentCore Runtime deployment, AgentCore Memory integration, per-agent usage accounting, or Gemma, Veo, or Lyria execution paths.
...
1. Preserve SQS and DynamoDB as Harmonia's durable workflow control plane.
2. Deploy the Strands cognitive team to AgentCore Runtime Runtime for managed agent execution.
3. Add AgentCore Memory for secure, brand-scoped context across jobs and operator sessions.
4. Add OpenTelemetry traces spanning web requests, SQS transitions, worker stages, agent delegations, model calls, validation, media operations, publishing, and verification.
...
- AgentCore Runtime does not replace the SQS stage machine, DynamoDB job state, media worker, policy engine, publisher, or verifier.
- AgentCore Memory does not become the source of truth for jobs, approvals, receipts, or facts about external actions.
...
```mermaid
flowchart TB
    UI[Next.js dashboard and chat] --> API[ECS Fargate web API]
    API --> FS[(DynamoDB durable state)]
    API --> PS[[SQS stage topic]]

    PS --> WORKER[ECS Fargate stage worker]
    WORKER --> FS
    ...
    WORKER --> AER[AgentCore Runtime Runtime]
    ...
    API -. trace context .-> TRACE[CloudWatch traces and Monitoring]
...
SQS plus DynamoDB already provides explicit stages, retry control, permanent-failure visibility, resumability, and an auditable approval boundary. AgentCore Runtime adds a managed home for cognitive execution, sessions, memory, evaluation, and traces. Keeping these concerns separate avoids turning a model runtime into a fragile business-process engine.

The ECS Fargate worker remains responsible for large media files, ffmpeg, polling media operations, deterministic action execution, and independent verification. AgentCore Runtime remains responsible for model delegation and structured judgment.
...
- `LocalAdkTeamRuntime`: Strands Runner plus invocation-scoped session service for local development and deterministic tests.
- `AgentEngineTeamRuntime`: calls the deployed Strands application, propagates trace context, validates returned state, and never performs a model fallback.
...
AgentCore Memory supplements DynamoDB; it does not replace it.
...
Each request includes the durable source event identifiers so a memory can be traced back to DynamoDB evidence. Memory generation failure does not roll back an already verified external action; it records a visible learning-stage failure.

## OpenTelemetry and Agent Observability

### Trace propagation

The web service creates or continues W3C trace context. SQS messages carry `traceparent` and `tracestate` attributes. The worker extracts that context before opening the stage span. Calls to AgentCore Runtime, media models, storage, publishing, and verification remain children of the same job trace where supported.

DynamoDB stage events store `traceId` and the relevant root `spanId` as additive audit metadata so an operator can move from a business event to CloudWatch traces.
...
- AgentCore Runtime and ECS Fargate use separate service identities with minimum required permissions.
...
- Existing DynamoDB documents remain readable.
...
3. **Managed cognitive runtime and memory** — AgentCore Runtime deployment/caller, sessions, scoped AgentCore Memory reads/writes, and failure semantics.
...
5. **Evidence and documentation** — architecture diagrams, live model comparison, latency/cost report, Cloud traces, AgentCore Memory evidence, and an updated four-minute demo.
...
- Trace context survives SQS serialization.
...
- Local and AgentCore Runtime runtimes return identical validated contracts.
- AgentCore Runtime missing outputs fail visibly.
- AgentCore Memory failures follow the specified stage semantics.
- Generated-media responses normalize into durable asset records.
- Existing DynamoDB fixtures remain readable after additive fields.

### Integration tests

- One real multimodal analysis uses source media plus transcript.
- One AgentCore Runtime invocation produces a correlated CloudWatch traces.
...
- AgentCore Runtime deployment resource and successful invocation
- Correlated CloudWatch traces covering web, SQS, worker, and agent spans
...
- Full Harmonia ECS Fargate and DynamoDB workflow shown in the demo

## Success Criteria

The design is complete when Harmonia can demonstrate one real job in which:

1. SQS and DynamoDB run the durable workflow.
2. The ECS Fargate worker invokes the Strands team on AgentCore Runtime.
3. Sophia analyzes actual multimodal source evidence.
4. At least two different model families perform distinct roles.
5. A later job retrieves scoped AgentCore Memory context derived from approved or verified evidence.
6. CloudWatch traces shows the correlated execution graph without raw content.
