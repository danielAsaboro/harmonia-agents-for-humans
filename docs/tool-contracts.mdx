...
description: "The complete least-privilege contract for tools exposed to Harmonia's Strands liaison."
---

Only Nova, the read-only liaison, receives these tools. No model-accessible tool can approve, publish, retry a job, mutate credentials or budgets, delete data, or write DynamoDB. The deterministic workflow owns all effects.
