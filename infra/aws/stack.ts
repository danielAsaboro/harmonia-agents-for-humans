...
export class HarmoniaAwsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const table = new dynamo.Table(this,"State",{partitionKey:{name:"pk",type:dynamo.AttributeType.STRING},sortKey:{name:"sk",type:dynamo.AttributeType.STRING},billingMode:dynamo.BillingMode.PAY_PER_REQUEST,pointInTimeRecoverySpecification:{pointInTimeRecoveryEnabled:true},removalPolicy:cdk.RemovalPolicy.RETAIN});
    const bucket = new s3.Bucket(this,"Assets",{blockPublicAccess:s3.BlockPublicAccess.BLOCK_ALL,enforceSSL:true,versioned:true,encryption:s3.BucketEncryption.S3_MANAGED,removalPolicy:cdk.RemovalPolicy.RETAIN,lifecycleRules:[{prefix:"quarantine/",expiration:cdk.Duration.days(7),abortIncompleteMultipartUploadAfter:cdk.Duration.days(1)}],cors:[{allowedOrigins:[publicUrl.valueAsString],allowedMethods:[s3.HttpMethods.PUT,s3.HttpMethods.GET,s3.HttpMethods.HEAD],allowedHeaders:["*"],exposedHeaders:["ETag","x-amz-version-id"]}]});
    const queues: Record<string,sqs.Queue> = {};
    for (const kind of ["stage","production","data","control"]) {
      const dlq = new sqs.Queue(this,`${kind}DeadLetters`,{retentionPeriod:cdk.Duration.days(14),enforceSSL:true});
      queues[kind] = new sqs.Queue(this,`${kind}Queue`,{visibilityTimeout:cdk.Duration.minutes(2),retentionPeriod:cdk.Duration.days(14),deadLetterQueue:{queue:dlq,maxReceiveCount:5},enforceSSL:true});
    }
    const token = new secrets.Secret(this,"InternalToken",{generateSecretString:{passwordLength:48,excludePunctuation:true}});
    const scannerToken = new secrets.Secret(this,"ScannerToken",{generateSecretString:{passwordLength:48,excludePunctuation:true}});
    const connectionKey = new secrets.Secret(this,"ConnectionKey",{generateSecretString:{passwordLength:32,excludePunctuation:true},description:"32-byte ASCII key; application accepts base64-encoded key through CONNECTION_KEY_SECRET_ARN"});
    const integrationSecrets = new secrets.Secret(this,"Integrations",{description:"Operator-configured integration credentials JSON; empty until explicitly connected",secretStringValue:cdk.SecretValue.unsafePlainText("{}")});
    ...
    workerTask.addContainer("worker",{image:ecs.ContainerImage.fromAsset(path.resolve(__dirname,"../../agent"),{exclude:[".venv","node_modules","__pycache__",".pytest_cache",".env*"],ignoreMode:cdk.IgnoreMode.GLOB}),environment:{...common,HARMONIA_ENABLE_QUEUE_CONSUMERS:"true"},secrets:runtimeSecrets,logging:ecs.LogDrivers.awsLogs({streamPrefix:"worker",logRetention:logs.RetentionDays.ONE_MONTH}),portMappings:[{containerPort:8080}]});
    const worker = new ecs.FargateService(this,"Worker",{cluster,taskDefinition:workerTask,desiredCount:1,minHealthyPercent:100,circuitBreaker:{rollback:true},cloudMapOptions:{name:"worker"}});
    ...
    for(const task of [webTask,workerTask]) {
      ...
      task.addContainer("otel",{image:ecs.ContainerImage.fromRegistry("amazon/aws-otel-collector:v0.46.0@sha256:371d3c8b9b97d0ebd0c56eb6b1e4f3d2ba4980c255f67d02695158e5851c2b31"),command:["--config=env:OTEL_CONFIG"],environment:{AWS_REGION:this.region,OTEL_CONFIG:collectorConfig},memoryReservationMiB:128,essential:true,logging:ecs.LogDrivers.awsLogs({streamPrefix:"otel",logRetention:logs.RetentionDays.ONE_MONTH})});
    }
    scannerTask.addContainer("scanner",{image:ecs.ContainerImage.fromAsset(path.resolve(__dirname,"../malware-scanner")),secrets:{MALWARE_SCANNER_TOKEN:ecs.Secret.fromSecretsManager(scannerToken)},logging:ecs.LogDrivers.awsLogs({streamPrefix:"scanner",logRetention:logs.RetentionDays.ONE_MONTH}),portMappings:[{containerPort:8080}]});
    const scanner = new ecs.FargateService(this,"Scanner",{cluster,taskDefinition:scannerTask,desiredCount:1,minHealthyPercent:100,circuitBreaker:{rollback:true},cloudMapOptions:{name:"scanner"}});
