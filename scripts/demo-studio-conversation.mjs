...
export function buildDemoStudioConversation({ runId }) {
  const draft = { id: "d1", platform: "x", text: "Your signup flow is an obstacle course. Ours was too — until we treated every step as a suspect.", valid: true, momentId: "m2", angleId: "a1" };
  ...
  return [
    ...
    ...pair("The voice should feel contrarian, not corporate.", "the narrative direction is founder conviction with a concrete proof point.", { intent: "list_drafts", jobId: "demo-clips", drafts: [] }),
    ...pair("Write it like a founder talking to another founder.", "one reviewed X draft is linked to the working set.", { intent: "list_drafts", jobId: "demo-clips", drafts: [draft] }),
    ...pair("Keep the opening about the obstacle course.", "the reviewed draft keeps that hook and remains inside the X limit.", { intent: "list_drafts", jobId: "demo-clips", drafts: [draft] }),
