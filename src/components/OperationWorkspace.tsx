...
export default function OperationWorkspace() {
  ...
  return <DashboardPage eyebrow="Current authority" title="Content operation" description="A read-only view of the persisted strategy, planned work, approvals, and measured results.">
    ...
    {state.status === "ready" && operation ? <div className="space-y-5" aria-live="polite">
      ...
      <section className="dash-panel"><h2>Proposed changes</h2>{operation.proposedChanges.length ? <ul>{operation.proposedChanges.map(change => <li key={change.id}><strong>{change.kind} · {change.id}</strong> · {change.status}{change.revision ? ` · revision ${change.revision}` : ""}{change.decision ? ` · decision: ${change.decision}` : ""}<br />Reviewable changes: {change.changes.join("; ") || "unavailable"}{change.evidenceRefs.length ? <><br />Evidence: {change.evidenceRefs.join(", ")}</> : null}</li>)}</ul> : <p>No current proposed changes.</p>}</section>
