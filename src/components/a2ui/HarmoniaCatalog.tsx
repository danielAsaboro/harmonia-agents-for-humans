...
const CampaignBriefApi: ComponentApi = { name: "CampaignBrief", schema: z.object({ jobId: z.string().min(1), title: z.string().min(1), brief: z.string(), sourceKind: z.enum(["written", "video", "audio", "mixed"]), platforms: z.array(z.string()).max(10), angles: z.array(z.object({ id: z.string().min(1), kind: z.enum(["source", "trend", "meme", "performance", "memory"]), title: z.string(), rationale: z.string() }).strict()).max(20), ...generatedNode }).strict() };
