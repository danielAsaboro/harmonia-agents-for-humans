...
export function ConfirmationCard({ title, description, risk, state, disabled, onDecision }: { title: string; description?: string; risk: "low" | "material" | "high"; state: "pending" | "approved" | "rejected" | "expired"; disabled?: boolean; onDecision?: (decision: "approved" | "rejected") => void }) {
  return (
    <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-xs dark:border-amber-800 dark:bg-amber-950/30"><div className="flex items-center justify-between gap-2"><strong>{title}</strong><span className="uppercase text-amber-700 dark:text-amber-400">{risk} risk</span></div>{description && <p className="mt-1 leading-5 text-zinc-600 dark:text-zinc-300">{description}</p>}{state === "pending" && onDecision ? <div className="mt-3 flex gap-2"><button type="button" disabled={disabled} onClick={() => onDecision("approved")} className="rounded-full bg-emerald-600 px-3 py-1 font-medium text-white disabled:opacity-50">Approve</button><button type="button" disabled={disabled} onClick={() => onDecision("rejected")} className="rounded-full bg-zinc-200 px-3 py-1 font-medium text-zinc-800 disabled:opacity-50 dark:bg-zinc-800 dark:text-white">Reject</button></div> : <p className="mt-2 font-medium capitalize">{state}</p>}</div>
  );
}
