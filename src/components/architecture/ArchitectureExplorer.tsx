...
function ExplorerInner() {
  ...
  const layout = layoutState.result; const layingOut = layoutState.key !== projectionKey;
  ...
  useEffect(() => { let active = true; layoutArchitecture(projection, "desktop").then((result) => { if (!active) return; setLayoutState({ key: projectionKey, result }); if (shouldFit.current) requestAnimationFrame(() => instance.current?.fitView({ padding: 0.16, duration: 500, minZoom: 0.38 })); shouldFit.current = false; }); return () => { active = false; }; }, [projection, projectionKey]);
  ...
  return <div className="architecture-shell">
    ...
    <main className="arch-stage">
      <div className="arch-graph"><ReactFlow<Node, Edge> nodes={layout.nodes} edges={layout.edges.map((edge) => { const kind = (edge.data?.kind ?? "workflow") as keyof typeof edgeAppearance; return { ...edge, markerEnd: { type: MarkerType.ArrowClosed, color: edgeAppearance[kind].color } }; })} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onInit={(flow) => { instance.current = flow; }} onNodeClick={(_, graphNode) => select(graphNode.id)} nodesDraggable={false} fitView fitViewOptions={{ padding: 0.16, minZoom: 0.38 }} minZoom={0.3} maxZoom={1.8}><Background color="#d4d2c7" gap={24} size={1} /><Controls showInteractive={false} /><MiniMap pannable zoomable nodeColor="#6e7c70" maskColor="rgba(241,240,231,.66)" /></ReactFlow>{layingOut ? <div className="arch-layout-state">Recalculating architecture…</div> : null}</div>
