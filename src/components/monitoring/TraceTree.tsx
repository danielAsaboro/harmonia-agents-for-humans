...
export default function TraceTree({ records }: { records: AgentActivity[] }) {
  ...
  return <div className="space-y-3">{[...grouped.entries()].map(([traceId, items]) => {
    ...
    return <section key={traceId} className="rounded-xl border border-zinc-200 p-3 dark:border-zinc-800"><p className="mb-1 break-all font-mono text-[11px] text-zinc-500">trace {traceId}</p><ul>{roots.map((root) => <TraceNode key={root.id} item={root} childMap={children} />)}</ul></section>;
  })}</div>;
}
