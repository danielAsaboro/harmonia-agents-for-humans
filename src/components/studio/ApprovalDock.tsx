    ...
    <aside className="border-t-2 border-[#161512] bg-[#fffdf7] p-3 shadow-[0_-14px_34px_rgba(22,21,18,0.13)]" aria-label="Approval boundary">
      ...
      {approvalOperations.length ? <HarmoniaA2uiHost operations={approvalOperations} onAction={(action) => {
        if (action.name === "decide_operation" && onOperationDecision) {
          ...
          if (operationId && (decision === "approved" || decision === "rejected")) void onOperationDecision(operationId, decision);
        }
        if (action.name === "decide_job_action") {
          ...
          const decision = action.context.decision;
          if (actionJobId && actionId && (decision === "approved" || decision === "rejected")) void onDecide(actionJobId, actionId, decision);
        }
      }} /> : null}
      {pending.map((action) => {
        ...
        return (
          <details key={action.id} data-action-id={action.id} open={pending.length === 1} className="group">
            ...
            <section className="mx-2 mb-2 grid gap-4 border border-black/15 bg-[#f4f0e8] p-4 xl:grid-cols-[1fr_auto]">
              ...
              <div className="flex items-end gap-2"><button type="button" disabled={busy} onClick={() => void onDecide(jobId, action.id, "rejected")} className="rounded-full border-2 border-[#161512] px-5 py-2.5 text-xs font-black uppercase tracking-wider disabled:opacity-40">Reject</button><button type="button" disabled={busy} onClick={() => void onDecide(jobId, action.id, "approved")} className="rounded-full bg-[#161512] px-5 py-3 text-xs font-black uppercase tracking-wider text-white shadow-[4px_4px_0_#d9ff43] disabled:opacity-40">Approve</button></div>
            </section>
          </details>
        );
      })}
