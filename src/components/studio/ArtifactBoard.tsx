  ...
  return (
    <div className="space-y-5">
      <section className="relative overflow-hidden border-2 border-[#161512] bg-[#fffdf7] p-6 shadow-[9px_9px_0_#161512]">
        ...
        <div className="relative max-w-2xl"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-[#3157ff]">Working set · {job.id}</p><h2 className="mt-3 max-w-xl font-serif text-4xl leading-[0.95]">{job.config.brief || job.config.youtubeUrl || "Untitled content direction"}</h2><div className="mt-5 flex flex-wrap gap-2"><span className="bg-[#161512] px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-white">{job.stage}</span><span className="border border-black/20 bg-white/70 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider">{job.status}</span>{model.pendingActions.length ? <span className="bg-[#ff5c35] px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-white">{model.pendingActions.length} decisions</span> : null}</div></div>
      </section>
