        ...
        return (
          <article key={asset.actionId} tabIndex={-1} data-canvas-artifact={artifactId} onClick={() => onSelect(artifactId)} className={`overflow-hidden border-2 bg-[#fffdf7] transition ${selectedArtifactId === artifactId ? "border-[#3157ff] shadow-[7px_7px_0_#3157ff]" : "border-black/15 hover:border-black/45"}`}>
            <div className="relative bg-[#161512]">
              ...
              {asset.mime.startsWith("video/") ? <video src={src} controls aria-label={asset.title} className="aspect-video w-full bg-black object-contain" /> : null}
            ...
            <div className="flex items-center gap-3 p-4"><div className="min-w-0 flex-1"><h3 className="truncate font-serif text-lg">{asset.title}</h3><p className="mt-1 font-mono text-[10px] uppercase text-black/40">{asset.mime} · {(asset.sizeBytes / 1_048_576).toFixed(2)} MB</p></div><span className="text-xl">↗</span></div>
          </article>
        );
      })}
    </div>
