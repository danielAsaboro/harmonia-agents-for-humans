...
export function SourcesWorkspace({ job, receipts }: { job: JobFull; receipts: Receipt[] }) {
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1.35fr_1fr]">
      <section className="border border-black/15 bg-[#fffdf7] p-5"><div className="mb-4 flex items-end justify-between"><div><p className="text-[9px] font-black uppercase tracking-[0.18em] text-[#3157ff]">Ground truth</p><h3 className="font-serif text-2xl">Normalized sources</h3></div><span className="font-mono text-[10px] text-black/40">{normalizedSegments.length} segments</span></div><ol className="max-h-[56vh] space-y-3 overflow-y-auto pr-2">{normalizedSegments.map(({ source, segment }) => <li key={`${source.sourceId}:${segment.id}`} data-canvas-artifact={`source:${source.sourceId}:${segment.id}`} className="border-t border-black/10 pt-3"><span className="font-mono text-[9px] text-[#ff5c35]">{source.title} · {segment.locator.kind}</span><p className="mt-1 text-sm leading-6 text-black/70">{segment.text}</p><code className="mt-1 block text-[8px] text-black/40">{JSON.stringify(segment.locator)}</code></li>)}</ol>{!normalizedSegments.length ? <p className="py-8 text-sm text-black/40">Sources are being collected and extracted. Failures pause for operator resolution.</p> : null}</section>
      <div className="space-y-5">
        {job.stage === "awaiting_source_resolution" ? <SourceResolutionPanel jobId={job.id} sources={job.sourceRecords ?? []} /> : null}
        {job.campaignOutputPlan ? <section className="border border-black/15 bg-[#e8f1ff] p-5"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-[#3157ff]">Campaign output plan</p><h3 className="font-serif text-xl">{job.campaignOutputPlan.outputs.length} grounded output{job.campaignOutputPlan.outputs.length === 1 ? "" : "s"}</h3><div className="mt-3 flex flex-wrap gap-2">{job.campaignOutputPlan.outputs.map((output) => <span key={output.id} className="rounded-full border border-black/15 bg-white px-2 py-1 text-[9px]">{output.outputType} · {output.approvalClass}</span>)}</div><code className="mt-3 block break-all text-[8px] text-black/40">{job.campaignOutputPlan.digest}</code>{job.campaignOutputPlan.desiredOutputs.some((item) => !job.campaignOutputPlan!.outputs.some((output) => output.outputType === item)) ? <p className="mt-2 text-[10px] text-amber-700">Some requested outputs are ineligible for the available evidence or outside the allowed set.</p> : null}</section> : null}
