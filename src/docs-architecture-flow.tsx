...
const nodeLinks: Record<string, { href: string; label: string }> = {
  ...
  ingest: { href: "/pipeline", label: "Ingestion pipeline" },
  transcribe: { href: "/pipeline", label: "Transcription pipeline" },
...
const allNodes: FlowNode[] = [
  ...
  n("ingest", 0, 600, "Ingest media", "01", "Resolve an authorized YouTube URL or upload.", ["workflow"]),
  n("transcribe", 230, 600, "Transcribe", "02", "Timed transcript and source provenance.", ["workflow"]),
...
const allEdges: FlowEdge[] = [
  ...
  ...["ingest","transcribe","analyze","strategy","strategyGate","plan","draft","effectGate","execute"].slice(0,-1).map((source, i) => e(`wf${i}`, source, ["ingest","transcribe","analyze","strategy","strategyGate","plan","draft","effectGate","execute"][i+1], i === 3 || i === 6 ? "request approval" : "durable artifact", ["workflow"], true)),
