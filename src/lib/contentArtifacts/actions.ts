...
export function deriveArtifactActions(artifacts: ContentArtifact[], connections: { x: boolean; linkedinDestination: Destination | null }): ActionSeed[] {
  const actions: ActionSeed[] = artifacts.map((artifact) => ({ id: `export-${artifact.id}-${artifact.contentDigest.slice(0, 12)}`, jobId: artifact.jobId, type: "export_content_artifact", title: `Export ${artifact.title}`, description: `Write and independently verify ${artifact.outputType.replaceAll("_", " ")} Markdown and JSON.`, payload: { type: "export_content_artifact", artifactId: artifact.id, artifactDigest: artifact.contentDigest } }));
