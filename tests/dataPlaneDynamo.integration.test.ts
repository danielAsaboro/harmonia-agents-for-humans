...
describe.skipIf(!emulator)("data-plane DynamoDB fencing", () => {
  it("creates a manifest batch and fences claim finalization", async () => {
    ...
    const batch = {
      ...
      manifest: { uri: `s3://${workspaceId}-artifacts/manifests/batch-1.json`, sha256: "a".repeat(64), itemCount: 1, byteCount: 500 },
  ...
  it("initializes beyond one DynamoDB transaction limit in resumable chunks", async () => {
    const repository = new DataPlaneRepository(db());
    const createdAt = "2026-08-31T00:10:00.000Z";
    const batch = {
      id: "batch-large", workspaceId, brandId: scope.brandId,
      manifest: { uri: `s3://${workspaceId}-artifacts/manifests/batch-large.json`, sha256: "d".repeat(64), itemCount: 501, byteCount: 50_100 },
    ...
    await runWithTenant(scope, () => repository.create(batch, work));
    const batchRef = recordKey(`workspaces/${workspaceId}/data_batches/${batch.id}`);
    expect(field((await awsRepository().read(batchRef)).value, "state")).toBe("pending");
    expect((await awsRepository().query(partition(batchRef.path + "/" + "work_items"))).size).toBe(501);
  });
