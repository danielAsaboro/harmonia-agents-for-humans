...
import { claimEffect, db, finalizeEffectReceipt, getEffectClaim, getJob, listReceipts } from "@/lib/repository";
...
describe.skipIf(!emulator)("effect claim DynamoDB transaction", () => {
  it("rejects an approved action flag without a durable human approval decision", async () => {
    ...
    await awsRepository().put(recordKey(forgedJobPath), {
    ...
    await db().removeTree(recordKey(forgedJobPath));
  });
  ...
  it("rejects an approval record without authenticated human actor provenance", async () => {
    ...
    await awsRepository().put(recordKey(partialJobPath), {
      workspaceId: scope.workspaceId, brandId: scope.brandId, createdByUserId: "operator-test",
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), status: "running",
      stage: "publish", config: { platforms: [] }, actions: [partialAction],
    });
    await awsRepository().put(recordKey(`${partialJobPath}/approval_decisions/${partialAction.id}`), {
      id: partialAction.id,
      jobId: partialJobId,
    ...
    await db().removeTree(recordKey(partialJobPath));
  });
  ...
  it("grants one concurrent owner and atomically links claim, receipt, and action", async () => {
    ...
    await awsRepository().put(recordKey(`${jobPath}/approval_decisions/${approvedAction.id}`), {
  ...
  it("reclaims a crashed pre-dispatch owner and still finalizes exactly one receipt", async () => {
    ...
    await awsRepository().put(recordKey(recoveryJobPath), {
      workspaceId: scope.workspaceId, brandId: scope.brandId, createdByUserId: "operator-test",
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), status: "running",
    ...
    await awsRepository().patch(recordKey(`${recoveryJobPath}/effect_claims/${key}`), {
    ...
    await db().removeTree(recordKey(recoveryJobPath));
  });

  afterAll(async () => {
    if (emulator) await db().removeTree(recordKey(jobPath));
  });
});
