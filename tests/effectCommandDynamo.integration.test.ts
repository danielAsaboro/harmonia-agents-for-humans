...
import { db, getDurableOperation, getJob, listReceipts } from "@/lib/repository";
...
describe.skipIf(!emulator)("effect command DynamoDB aggregate", () => {
  it("grants one owner and atomically finalizes command, claim, receipt, and action", async () => {
    ...
    const approvalRef = recordKey(`${jobPath}/approval_decisions/${action.id}`);
    await awsRepository().put(approvalRef, { ...approval(action), actorType: "service", channel: "telegram" });
    ...
    await awsRepository().put(approvalRef, approval(action));
  ...
  it("atomically quarantines a post-dispatch ambiguity with its operation", async () => {
    ...
    await awsRepository().put(recordKey(`workspaces/${scope.workspaceId}/jobs/${unknownJobId}`), {
      workspaceId: scope.workspaceId, brandId: scope.brandId, createdByUserId: "operator-test",
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), status: "running",
    ...
    await awsRepository().put(recordKey(`workspaces/${scope.workspaceId}/jobs/${unknownJobId}/approval_decisions/${unknownAction.id}`), approval(unknownAction));
  ...
  it("reclaims only a dispatch proven not to have entered the provider", async () => {
    ...
    await awsRepository().put(recordKey(`workspaces/${scope.workspaceId}/jobs/${retryJobId}`), {
      workspaceId: scope.workspaceId, brandId: scope.brandId, createdByUserId: "operator-test",
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), status: "running",
    ...
    await awsRepository().put(recordKey(`workspaces/${scope.workspaceId}/jobs/${retryJobId}/approval_decisions/${retryAction.id}`), approval(retryAction));
