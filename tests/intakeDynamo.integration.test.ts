...
describe.skipIf(!process.env.AWS_LOCAL_ENDPOINT)("durable intake commands", () => {
  ...
  it("dispatches a text-only strategy with real Python serialization and no manufactured evidence", () => run(async () => {
    ...
    await configurePlanningPolicy({ timezone: "UTC", productionCapacity: { maxItems: 8, maxItemsPerWeek: 4 }, cadenceConstraints: { minimumHoursBetweenItems: 0, maxItemsPerChannelPerWeek: 4 }, maxConcurrentItems: 1 }, 0);
    const initiatives = [];
    for (let index = 0; index < 2; index++) {
      const initiative = await submitIntakeTurn({ ...turn(), advice: { ...advice, disposition: "new_initiative", expectedOutcome: "Launch", requestedOutputs: ["linkedin_post"], sourceHandles: [] } });
      initiatives.push(await materializeIntake({ draftId: initiative.id, expectedDraftRevision: initiative.revision, requestId: initiative.answers.at(-1)!.requestId }));
    }
    const target = initiatives[1];
    const placement = { ...turn(), advice: { ...advice, sourceHandles: [], requestedOutputs: ["linkedin_post" as const], disposition: "existing_plan_item" as const, targetName: "Launch" } };
    ...
    const resolved = await submitIntakeTurn({ ...placement, requestId: randomUUID(), message: `${target.planRef.id}/${target.itemRefs[0].id}`, advice: { ...placement.advice, targetName: `${target.planRef.id}/${target.itemRefs[0].id}` } });
    expect(resolved.id).toBe(ambiguous.id);
    expect(resolved.target).toEqual({ campaignId: target.campaignRef!.id, planId: target.planRef.id, itemId: target.itemRefs[0].id, name: "Launch" });
    const placed = await executeIntakeDraft(resolved);
    expect(placed.planning).toEqual(target);
