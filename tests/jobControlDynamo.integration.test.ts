...
import { createJob, db, getJob, listDispatchableStageOutbox } from "@/lib/repository";
...
const scope = {
  workspaceId,
  brandId: "brand-test",
  principal: cognitoPrincipal({ subjectId: "user-test", workspaceRole: "owner", authenticationId: "firebase-test" }),
};

describe.skipIf(!emulator)("job control DynamoDB transaction", () => {
  it("records one receipt and returns it for an identical command retry", async () => {
    ...
    const command = createCommandEnvelope({
      ...
      actor: { actorType: "cognito_operator", subjectId: "user-test", authenticationId: "firebase-test" },
    ...
    await awsRepository().patch(recordKey(`workspaces/${workspaceId}/jobs/${job.id}`), { actions: [action] });
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/jobs/${job.id}/approval_decisions/${action.id}`), { id: "inflight-approval", jobId: job.id, actionId: action.id, decision: "approved", payloadDigest: actionPayloadDigest(action), actorType: "cognito_operator", actorSubjectId: "user-test", authenticationId: "firebase-test", channel: "dashboard", operationId: `${job.id}:approval:${action.id}`, traceId: "f".repeat(32), decidedAt: new Date().toISOString() });
