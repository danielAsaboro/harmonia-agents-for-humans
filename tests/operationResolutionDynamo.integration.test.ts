import { recordKey, field, awsRepository } from "../src/lib/dynamo";
...
describe.skipIf(!emulator)("unknown effect operator resolution", () => {
  it("atomically binds evidence, resolution, receipt, command, claim, operation, and job", async () => {
    ...
    await Promise.all([
      awsRepository().put(recordKey(jobPath), {
      ...
      awsRepository().put(recordKey(`${tenantCollectionPath(scope, "operations")}/${operationId}`), operation),
      awsRepository().put(recordKey(commandPath), command),
      awsRepository().put(recordKey(`${tenantCollectionPath(scope, "artifacts")}/${artifact.id}`), artifact),
    ]);
    await awsRepository().put(recordKey(`${commandPath}/claims/effect`), claim);
    ...
    const [storedOperation, storedJob, receipts, storedCommand, storedClaim] = await runWithTenant(scope, async () => Promise.all([
      getDurableOperation(operationId), getJob(jobId), listReceipts(jobId), awsRepository().read(recordKey(commandPath)), awsRepository().read(recordKey(`${commandPath}/claims/effect`)),
    ]));
    expect(storedOperation?.state).toBe("succeeded");
    expect(storedJob.actions[0].state).toBe("executed");
    expect(receipts).toHaveLength(1);
    expect(field(storedCommand.value, "state")).toBe("applied");
    expect(storedClaim.value).toMatchObject({ state: "applied", receiptId: receipts[0].id });
  });
