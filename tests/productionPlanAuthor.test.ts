...
describe("production plan author", () => {
  it("turns a grounded creative draft into a supported, exactly quoted first revision", async () => {
    ...
    expect(plan).toMatchObject({
      ...
      estimatedCostUsd: "0.860000", maximumCostUsd: "0.860000",
    });
    expect(Object.values(plan.operationCostsUsd)).toEqual(["0.320000", "0.480000", "0.060000"]);
    expect(plan.soundtrack).toMatchObject({ modelCapability: "lyria-2", instrumental: true, targetDurationSec: 10 });
  ...
  it("keeps the plan identity and increments the revision when revising", async () => {
    ...
    const plan = await authorProductionPlan({
      ...
      existing: { id: "media-existing", revision: 3 } as never, generateDraft,
    });
