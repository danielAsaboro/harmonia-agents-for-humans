import { recordKey, awsRepository, partition } from "../src/lib/dynamo";
...
const videoSpec = generatedVideoSpecSchema.parse({ modelCapability: "nova-reel", mode: "text_to_video", prompt: "Calm blue network", durationSec: 6, aspectRatio: "16:9", resolution: "720p", outputCount: 1 });
const musicSpec = generatedMusicSpecSchema.parse({ modelCapability: "elevenlabs-music", prompt: "Warm minimal electronic soundtrack", instrumental: true, targetDurationSec: 30, outputCount: 1 });
...
describe.skipIf(!emulator)("production plan DynamoDB aggregate", () => {
  it("persists plans with explicitly undefined optional media fields", async () => {
    ...
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/jobs/${optionalJobId}`), {
  ...
  it("schedules and materializes a digest- and rights-bound source operation", async () => {
    ...
    await awsRepository().put(recordKey(`workspaces/${workspaceId}`), { defaultBrandId: brandId });
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/jobs/${sourceJobId}`), {
    ...
    const sealedPlan = videoProductionPlanSchema.parse({
      ...
      id: sourcePlanId,
      jobId: sourceJobId,
      target: { ...basePlan.target, durationSec: 6 },
      scenes: [{
        ...sourceScene,
        sourceWindow: { startSec: 0, durationSec: 6 },
  ...
  it("does not claim paid Nova Reel conditioning until the exact resolver artifact succeeds", async () => {
    const unique = Date.now();
    const conditionedJobId = `production-conditioned-${unique}`;
    const conditionedPlanId = `plan-conditioned-${unique}`;
    await awsRepository().put(recordKey(`workspaces/${workspaceId}`), { defaultBrandId: brandId });
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/jobs/${conditionedJobId}`), {
  ...
  it("transactionally rejects a second production plan identity for one job", async () => {
    ...
    await awsRepository().put(recordKey(`workspaces/${workspaceId}`), { defaultBrandId: brandId });
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/jobs/${jobId}`), {
  ...
  it("atomically schedules and claims a cost-free composition after paid dependencies succeed", async () => {
    ...
    await awsRepository().put(recordKey(`workspaces/${workspaceId}`), { defaultBrandId: brandId });
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/jobs/${internalJobId}`), {
    ...
    await runWithTenant(serviceScope, () => startProductionProviderSubmission(
      internalPlan.id, paid.id, {
        claimId: paidClaim.claim.id, claimToken: "internal-paid-worker", provider: "nova_reel",
      },
    ));
    ...
    await runWithTenant(serviceScope, () => completePaidProductionOperation(
      internalPlan.id, paid.id, {
        ...
        providerMetadata: { provider: "nova_reel", providerOperationId: "operations/internal-video" },
      },
    ));
    ...
    const aggregateRef = recordKey(`workspaces/${workspaceId}/brands/${brandId}/production_plans/${internalPlan.id}`);
    await awsRepository().patch(aggregateRef, { currentRevision: 2, currentPlanDigest: "f".repeat(64), internalRun: 0 });
    ...
    await awsRepository().patch(aggregateRef, {
  ...
  it("rejects a claimed operation if its revision is superseded before provider submission", async () => {
    ...
    await awsRepository().put(recordKey(`workspaces/${workspaceId}`), { defaultBrandId: brandId });
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/jobs/${raceJobId}`), {
      workspaceId, brandId, status: "running", stage: "draft",
      createdAt: "2026-08-31T09:00:00.000Z", updatedAt: "2026-08-31T09:00:00.000Z",
    });
    ...
    expect(publishedWake).toMatchObject({ state: "published", transportMessageId: "production-message-race" });
    ...
    await awsRepository().patch(recordKey(`workspaces/${workspaceId}/brands/${brandId}/production_operation_outbox/${initialWake.id}`), { availableAt: "2000-01-01T00:00:00.000Z" });
    ...
    await awsRepository().patch(recordKey(`workspaces/${workspaceId}/brands/${brandId}/production_operation_outbox/${initialWake.id}`), { publishLeaseExpiresAt: "2000-01-01T00:00:00.000Z" });
    expect(await runWithTenant(serviceScope, () => listDispatchableProductionOutbox(
      100, new Date(),
  ...
  it("persists immutable revisions and invalidates the active mandate after a material revision", async () => {
    ...
    const initialOutbox = await awsRepository().query(partition(`workspaces/${workspaceId}/brands/${brandId}/production_operation_outbox`));
    expect(initialOutbox.rows.map((doc) => doc.value)).toEqual(expect.arrayContaining([
    ...
    const videoOutboxAfterPoll = await awsRepository().read(recordKey(`workspaces/${workspaceId}/brands/${brandId}/production_operation_outbox/${activeClaim.claim.id}`));
    expect(videoOutboxAfterPoll.value).toMatchObject({ state: "pending", availableAt: "2000-01-01T00:00:00.000Z" });
    ...
    expect((await awsRepository().read(recordKey(`workspaces/${workspaceId}/brands/${brandId}/production_operation_outbox/${activeClaim.claim.id}`))).value).toMatchObject({ state: "completed" });
    ...
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/brands/${brandId}/production_plans/${basePlan.id}/mandates/${mandate.id}`), expiredMandate);
    ...
    await awsRepository().patch(recordKey(`workspaces/${workspaceId}/brands/${brandId}/production_plans/${revisionTwo.id}/operation_claims/${revisionTwoClaim.claim.id}`), { leaseExpiresAt: "2000-01-01T00:00:00.000Z" });
  ...
  it("persists an immutable rejection decision with verified operator provenance", async () => {
    ...
    await awsRepository().put(recordKey(`workspaces/${workspaceId}/jobs/${rejectedJobId}`), {
    ...
    const decision = await awsRepository().read(recordKey(`workspaces/${workspaceId}/brands/${brandId}/production_plans/${rejectedPlan.id}/decisions/v1`));
