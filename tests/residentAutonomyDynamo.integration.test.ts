...
describe.skipIf(!emulator)("resident autonomy DynamoDB transactions", () => {
  ...
  afterAll(async () => { if (emulator) await db().removeTree(recordKey(`workspaces/${tenant.workspaceId}`)); });
});
