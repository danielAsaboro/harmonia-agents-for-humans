...
describe("state ownership documentation", () => {
  ...
  it.each([
    "Invocation state", "Managed Strands invocation", "DynamoDB job", "Approval decisions",
    "Receipts and verification", "Chat history", "AgentCore Memory", "Secrets", "Private evidence",
    "Source of truth", "Retention",
  ])("documents %s", (required) => expect(document).toContain(required));

  it("assigns editorial-plan persistence and lifecycle authority to DynamoDB and deterministic code", () => {
    ...
    expect(document).toMatch(/AgentCore Memory[\s\S]*never.*authoriz/i);
  });
