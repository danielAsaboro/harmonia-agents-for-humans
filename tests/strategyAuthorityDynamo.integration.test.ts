...
describe.skipIf(!process.env.AWS_LOCAL_ENDPOINT)("brand strategy authority", () => {
  ...
  it("approves the first strategy then plans and drafts a second pinned job with disjoint evidence", () => run(async () => {
    await configurePlanningPolicy({ timezone: "UTC", productionCapacity: { maxItems: 8, maxItemsPerWeek: 2 }, cadenceConstraints: { minimumHoursBetweenItems: 24, maxItemsPerChannelPerWeek: 2 }, maxConcurrentItems: 1 }, 0);
    ...
    await store.put(recordKey(`${root}/jobs/${second.id}/source_manifests/second-manifest`), sealManifest({ id: "second-manifest", jobId: second.id, revision: 1, directSourceIds: ["second-source"], excludedSourceIds: [], exclusionRecords: [], sealedAt: new Date().toISOString(), sealedBySubjectId: "operator" }));
    ...
    const accepted = await repository.acceptEditorialPlan(second.id, plan, 1);
    const executionJob = await repository.getJob(accepted.executionJobId!);
    const drafting = exercise("draft", executionJob);
    ...
    expect(drafting.observed[0].input.sourceBinding).toEqual({ ...snapshot.snapshot.sourceBinding, jobId: executionJob.id });
