...
function validBundle() {
  ...
  return {
    ...
    environment: {
      accountId: "project", region: "us-east-1", webService: "web", webRevision: "web-r1",
      ...
      dynamodbTableArn: "arn:aws:dynamodb:us-east-1:123456789012:table/harmonia", stageQueueArn: "arn:aws:sqs:us-east-1:123456789012:stages",
    ...
    events: stages.map((stage, index) => ({ eventId: `e-${index}`, stage, status: stage === "awaiting_approval" ? "waiting" : "completed", at: new Date(Date.parse("2026-08-24T12:01:00.000Z") + index * 60_000).toISOString(), operationId: `job:${stage}:0`, transportMessageId: `m-${index}`, traceId: index >= 5 ? approvalTraceId : workflowTraceId })),
    cognition: [{ role: "coordinator", model: "us.anthropic.claude-sonnet-4-6", provider: "bedrock", policyVersion: "v1", usageRecordId: "u-1", operationId: "job:understand:coordinator", traceId: workflowTraceId }],
    approval: { approvalId: "approval", actionId: "action", decision: "approved", actorType: "cognito_operator", decidedAt: "2026-08-24T12:10:00.000Z", traceId: approvalTraceId },
